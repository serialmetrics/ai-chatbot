...
export const useUploader = (strategy: Strategy, uploader: Uploader, oldOptions?: OldOptions) => {
    let { addFile, updateFileProgress, FileInput, openFileDialog, files, resetFiles } =
        useUploadFiles();

    let uploadToS3 = async (file: File, options: UploadToS3Options = {}) => {
        console.log('uploadToS3 start, file:', file, options)
...
let getUploadParams = async (strategy: Strategy, file: File, requestOptions?: RequestOptions) => {
    ...
    console.log('Request Upload Token:', body)
